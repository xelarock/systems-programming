...
Options parseArgs(int argc, char *argv[]){
	...
	while((o = getopt(argc, argv, "bg:i:r:smt:n:o:")) != -1) {        // If there is an option agument, parse it
	    switch (o) {
	...
	if (optind < argc){   // parses non-options args, aka input file location
	    for (; optind < argc; optind++){
            if (containsInput == 0){                             
            	opts.input = argv[optind];                       // set input file location and move to the next argument 
            	containsInput = 1;
	        }
	    }
    }
    ...
    if (containsInput == 0){                                    // check if output file given
        fprintf(stderr, "Error: No input file specified\n");
        exit(1);
    }
    if (containsOutput == 0){                                    // check if input file given
